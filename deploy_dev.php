...
// Only deploy on pushes to dev
$data = json_decode($payload, true);
if (($data['ref'] ?? '') !== 'refs/heads/dev') {
    exit('Not dev branch, skipping.');
}
...
$output = [];
$returnCode = 0;
exec("cd " . escapeshellarg($repoDir) . " && /usr/local/cpanel/3rdparty/bin/git pull origin dev 2>&1", $output, $returnCode);
